...
<script type="text/javascript">
	...
	<div class="container">
		...
		<div class="row justify-content-center">
			<div class="card shadow" style="border: none; margin-top: 40px; width: 80%">
				<!-- TAMPILAN PENGADUAN USER -->
				<form action="{{url('admin/viewTanggapan/store')}}" method="POST">
					{{csrf_field()}}
					@foreach($data as $array)
					<input type="hidden" name="inPengaduan" value="{{$array->id_pengaduan}}">
					@endforeach
					@foreach($petugas as $p)
					<input type="hidden" name="inPetugas" value="{{$p->id_petugas}}">
					@endforeach
					<div class="card-body">
						<textarea name="inIsi" placeholder="Ketik tanggapan..." class="form-control" rows="11"></textarea>
					</div>

					<div class="card-footer bg-white">
						<button class="btn bg-primary text-white" style="margin-right: 580px;">Submit</button>
					</div>
				</form>
			</div>
		</div>
