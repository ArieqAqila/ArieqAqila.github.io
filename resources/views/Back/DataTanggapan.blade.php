					...
					<table class="table table-bordered" id="myTable">
					  ...
					  <tbody>
					    ...
					    <tr>
					      ...
					      <td>{{$p->nik}}</td>
					      <td>{{ str_limit($p->isi_laporan,50,'...') }}</td>
					      <td>
					      	<?php echo date('d F Y', strtotime($p->tgl_pengaduan)); ?>
					      </td>
					      <td><a href="../data_gambar/{{$p->foto}}"><img width="100px" src="../data_gambar/{{$p->foto}}"></a></td>
					      <td>{{$p->status}}</td>
					      <th>
							<a href="{{url('admin/viewTanggapan/insert')}}/{{$p->id_pengaduan}}" class="btn btn-outline-primary">
								Beri Tanggapan!
							</a>
					      </th>
					    </tr>
